        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-white font-semibold text-sm truncate">
              Order &middot; {order.products.length} item
              {order.products.length !== 1 ? "s" : ""}
            </p>
            <p className="text-zinc-500 text-xs mt-0.5 font-mono truncate">
              {order._id}
            </p>
          </div>
        </div>
...
export default function ProfilePage() {
  ...
  const totalSpent = orders.reduce(
    (sum, order) =>
      ...
      order.products.reduce(
        (s, item) => s + (item.product?.selling_price ?? 0) * item.quantity,
        0,
      ),
    0,
  );
  ...
  return (
    <main className="min-h-screen pt-16" style={{ background: "#09090b" }}>
      {/* Subtle top gradient glow */}
      <div
        className="fixed top-0 left-0 right-0 h-64 pointer-events-none opacity-30"
        style={{
          background:
            "linear-gradient(180deg, rgba(39,39,42,0.8) 0%, transparent 100%)",
        }}
      />
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      >
                        <span
                          ...
                          style={{
                            background:
                              profile.role === "admin" ? "#f87171" : "#71717a",
                          }}
                        />
              ...
              {[
                ...
                },
                {
                  ...
                  value: orders.reduce(
                    (s, o) =>
                      s +
                      o.products.filter((p) => p.status === "delivered").length,
                    0,
                  ),
                  icon: (
                    <svg
