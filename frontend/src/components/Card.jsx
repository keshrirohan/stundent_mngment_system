...
const Card = ({ products }) => {
  const [likedProducts, setLikedProducts] = useState({});
    ...
    const handleLike = async (productId) => {
      try {
        const response = await fetch(`/api/liked/${productId}`, {
          method: "POST",
          headers: {
          ...
          >
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-3 right-3 p-2 rounded-full bg-zinc-800/80 backdrop-blur-sm transition-transform hover:scale-110"
              >
                <Heart
              ...
              {/* Stock Badge */}
              <span
                className="absolute bottom-2.5 left-2.5 text-xs font-semibold px-2.5 py-1 rounded-full"
                style={
                  product.isInStock
                    ? {
            ...
            <div className="p-4">
              ...
              <div className="flex items-center gap-1.5 mt-3">
                <span className="text-xs font-semibold px-1.5 py-0.5 rounded text-white bg-green-600">
                  4.5 ★
                </span>
                ...
                {product.mrp > product.selling_price && (
                  <>
                    ...
                    <span className="text-emerald-400 text-xs font-semibold">
                      Save ₹
                      {(product.mrp - product.selling_price).toLocaleString()}
                    </span>
                  </>
                )}
              </div>
